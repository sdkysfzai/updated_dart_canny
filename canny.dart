// ignore_for_file: non_nullable_equals_parameter, unrelated_type_equality_checks
...
Set<Set<Index2d>> canny(
  ...
  getSafe(x, y, Image image) {
    final pixel = image.getPixel(clampX(x), clampY(y));
    return pixel.r;
  }
  ...
  for (var y = 0; y < image.height; ++y) {
    for (var x = 0; x < image.width; ++x) {
      ...
      sobel.setPixelRgba(x, y, mag, mag, mag, mag);
      double direction = math.atan2(gy, gx);
      //convert to angle
      direction = (direction + math.pi / 2) * 180 / math.pi;
      if (direction >= 22.5 && direction < 67.5) {
        //45 deg
        edgeDirection.setPixelIndex(x, y, 45);
      } else if (direction >= 67.5 && direction < 112.5) {
        //90 deg
        edgeDirection.setPixelIndex(x, y, 90);
      } else if (direction >= 112.5 && direction < 157.5) {
        //135 deg
        edgeDirection.setPixelIndex(x, y, 135);
      } else {
        //0 deg
        edgeDirection.setPixelIndex(x, y, 0);
      }
  ...
  getNeighbours(x, y) {
    int direction = edgeDirection.getPixelIndex(x, y);
    Set<Index2d> nei = {};
    switch (direction) {
      case 0:
  ...
  for (var y = 0; y < image.height; ++y) {
    for (var x = 0; x < image.width; ++x) {
      ...
      if (max > p) {
        ...
        image.setPixelRgba(x, y, p, p, p, p);
      }
    }
  }
  ...
  for (var y = 0; y < image.height; ++y) {
    for (var x = 0; x < image.width; ++x) {
      ...
      if (!isWeak(x, y)) {
        //pixel is background
        labeledPixels.setPixelIndex(x, y, 1);
        image.setPixelRgba(x, y, 0, 0, 0, 0);
        continue;
      }
      ...
      while (currentBlobNeighbours.isNotEmpty) {
        ...
        labeledPixels.setPixelIndex(w.x, w.y, currentLabel);
  ...
  for (var w in nonEdges) {
    image.setPixelRgba(w.x, w.y, 0, 0, 0, 0);
  }
